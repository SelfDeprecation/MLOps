...
def memory_section(report: dict) -> list[str]:
    ...
    weights_mb = report["parameter_bytes"] / 1024 ** 2
    rss_values = [item["peak_rss_mb"] for item in modes.values()]
    rss_spread = max(rss_values) - min(rss_values)
    lines += [
        "",
        f"Вес параметров при dtype {report['dtype']} — {weights_mb:.0f} МБ. Full fine-tune добавляет",
        ...
        f"инференса. В этом замере LoRA требует "
        f"{modes['lora']['peak_mb'] / base['peak_mb']:.2f}× от инференса, а full fine-tune — "
        f"{modes['full_ft']['peak_mb'] / base['peak_mb']:.2f}×.",
        "",
    ]
...
def defects_section(report: dict) -> list[str]:
    """Разбор четырёх намеренных дефектов из выданного проекта."""
    ...
    return [
        ...
        "",
        "`named_parameters(remove_duplicate=False)` возвращает и embedding, и "
        "`lm_head`, хотя при `tie_word_embeddings=True` это один объект. Раньше "
        "каждая строка помечалась как уникальная. Теперь параметры отмечаются по "
        "идентичности объекта, а повтор учитывается в колонке «Разделяет тензор».",
        "",
        f"Повторно встречается {thousands(tied)} параметров. После исправления сумма "
        f"таблицы {thousands(report['params_total'])} в точности совпадает с "
        f"`sum(p.numel())` = {thousands(report['params_direct'])}.",
        "",
        "### 2. Forward-хуки оставались на слоях",
        "",
        "Регистрация хука возвращает handle, но исходный код его терял. Каждый новый "
        "вызов добавлял ещё три хука. Теперь handles сохраняются и удаляются в "
        "`finally`, включая путь с исключением.",
        "",
        f"После первого прогона зарегистрировано {hooks['after_first']} хуков, после "
        f"второго — {hooks['after_second']}; нормы двух прогонов "
        f"{'совпали' if hooks['repeat_equal'] else 'НЕ совпали'}.",
        "",
        "### 3. Вместо пика измерялся остаток после очистки",
        "",
        "Исходный `PeakMemory.__exit__` сначала вызывал сборщик мусора и только затем "
        "читал текущее потребление. Это стирало разницу между режимами. Теперь CPU "
        "использует системный high-water mark, CUDA — встроенный максимум аллокатора, "
        "а MPS опрашивается фоновым sampler-потоком. Каждый режим запускается в "
        "отдельном процессе, поэтому его пик не загрязнён предыдущим режимом.",
        "",
        f"Пики: инференс {memory['inference']['peak_mb']:.0f} МБ, LoRA "
        f"{memory['lora']['peak_mb']:.0f} МБ, full fine-tune "
        f"{memory['full_ft']['peak_mb']:.0f} МБ. Порядок "
        "`full fine-tune > LoRA > inference` восстановлен.",
        "",
        "### 4. На ускорителе использовалась RSS процесса",
        "",
        "RSS подходит для CPU, но не показывает тензоры в памяти CUDA/MPS. Теперь "
        "диспетчер метрики выбирает `torch.cuda.max_memory_allocated` для CUDA, "
        "`torch.mps.driver_allocated_memory` для MPS и RSS только для CPU.",
        "",
        f"На этой машине устройство `{base['device']}`, поэтому фактически использована "
        f"метрика `{base['metric_source']}`. Вес параметров модели равен "
        f"{weights_mb:.0f} МБ, а пик инференса {base['peak_mb']:.0f} МБ, то есть "
        "проверка «пик не меньше весов» выполняется. Числа для ускорителя не "
        "приводятся: на этой машине CUDA/MPS нет.",
        "",
    ]
