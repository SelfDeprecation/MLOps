...
class PeakMemory:
    """Пиковая память за весь режим, а не остаток после очистки."""
    ...
    def __enter__(self) -> "PeakMemory":
        ...
        if self.device.type == "cuda":
            ...
            self._sampler = threading.Thread(target=self._sample_until_stopped, daemon=True)
            self._sampler.start()
        return self
    ...
    for mode in MODES:
        ...
        for _ in range(repeats):
            ...
            if completed.returncode != 0:
                raise RuntimeError(
                    f"замер {mode} завершился с кодом {completed.returncode}:\n"
...
def main() -> None:
    ...
    if args.probe:
        # Служебный вывод должен одинаково читаться как UTF-8 и в Git Bash на
        # Windows, где консольная кодировка иначе может превратить русские поля
        # в CP1251-байты внутри JSON-файла.
    ...
    # Сначала дочерние процессы замера: родитель в этот момент не должен держать
    # ещё одну копию весов и искажать доступную память машины.
    ...
    activations_repeat = activation_norms(tokenizer, model, params)
    ...
    report = {
        ...
        "hook_check": {
            ...
            "repeat_equal": activations["norms"] == activations_repeat["norms"],
        },
